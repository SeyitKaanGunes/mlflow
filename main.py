...
from llm_utils import DEFAULT_LLM_MODEL, generate_metrics_summary
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Hugging Face model used for the lightweight LLM metrics summary.",
    )
    parser.add_argument(
        "--llm-max-new-tokens",
        type=int,
        default=72,
        help="Maximum number of new tokens generated by the LLM summariser.",
    )
    parser.add_argument(
        "--disable-llm-summary",
        action="store_true",
        help="Skip the LLM-powered metrics summary step.",
    )
...
def log_run_to_mlflow(
    ...
    with mlflow.start_run(run_name=run_name):
        mlflow.log_params(
            {
                ...
                "llm_summary_enabled": not args.disable_llm_summary,
            }
        )
        ...
        if llm_summary:
            llm_summary_path = artifacts_dir / "llm_metrics_summary.txt"
            llm_summary_path.write_text(
                llm_summary, encoding="utf-8", errors="replace"
            )
            mlflow.log_artifact(str(llm_summary_path))
...
def main() -> None:
    ...
    llm_summary = None
    if not args.disable_llm_summary:
        try:
            llm_summary = generate_metrics_summary(
                evaluation_outputs["metrics"],
